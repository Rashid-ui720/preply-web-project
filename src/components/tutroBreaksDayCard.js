...
class TutorBreakDayCard extends React.Component {
  ...
  toggle = async () => {
    ...
    if (this.state.collapse != true) {
      this.props.HandleDayOf_On(this.props.currentDay.current_day, false);
    } else {
      this.props.HandleDayOf_On(this.props.currentDay.current_day, true);
    }
  };

  componentDidMount() {
    //open or close  card accoridngly
    if (this.props.currentDay.is_off) {
        ...
        <div className="card border-0">
          ...
          >
            <h6 className="mb-0 font-size-4">
              {this.props.currentDay.current_day}
            </h6>
            <Toggle
...
export default TutorBreakDayCard;

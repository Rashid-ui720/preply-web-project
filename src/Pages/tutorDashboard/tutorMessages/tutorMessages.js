...
class TutorMessages extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      search: "",
  ...
  async componentDidMount() {
    ...
    if (query !== null) {
      ...
      this.setState({
        ...
        selectedChannelFriendlyName: ChannelfriendlyName.split("_and_")[1],
      });
  ...
  ChannelCheck = (instructor_id, student_id, channelFriendlyName) => {
    ...
    const channel_id = instructor_id.toString() + "_" + student_id.toString();
    ...
    TwilioService.getInstance()
      ...
      .then((client) => {
        client
          ...
          .catch((err) => {
            console.error("Create New Channel!", err);
            client
              .createChannel({
              ...
              .then(async (channel) => {
                ...
                this.addMember(student_id, channel_id);
                let params = {
                  student_user_id: parseInt(student_id),
  ...
  render() {
    return (
      <div
        className="dashboard-main-container mt-15 mt-lg-25"
        id="dashboard-body"
      >
...
export default connect(mapStateToProps, null)(TutorMessages);
